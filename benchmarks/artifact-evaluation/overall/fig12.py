...
def read_result_series(exp_name: str, file_prefix: str) -> list[ExpResult]:
    ...
    for i in range(len(new_results)):
        ...
        tpots.append(np.percentile([new_results[i].req_results[j].tpot for j in range(len(new_results[i].req_results))], 95))
...
faster_transformer = [0.16428, 1.803368,	5.184494,	16.268263] + [16]*12
_, vllm = read_result_series("logs", "exp-vllm-sharegpt")
vllm = vllm + [vllm[-1]] * 5
_, fastserve_fcfs = read_result_series("logs", "exp-fcfs-sharegpt")
fastserve_fcfs = fastserve_fcfs + [fastserve_fcfs[-1]] * 5
...
rate_y_ticks = [i * 0.1 for i in range(0, 7, 1)] # [150, 300, 450, 600]
print('rate')
times_1 = []
times_2 = []
...
# Save the figure
file_path = os.path.join(lib.get_exp_result_root(), "fig12.pdf")
