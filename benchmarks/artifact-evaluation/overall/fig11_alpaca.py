...
labels = {0: 'FasterTransformer', 1: 'vLLM', 2:'FastServe-FCFS', 3:'FastServe'}
...
faster_transformer = [0.13, 0.391, 0.81] + [0.81] * 11
_, vllm = read_result_series("logs", "exp-vllm-alpaca")
vllm = vllm + [vllm[-1]] * 2
_, fastserve_fcfs = read_result_series("logs", "exp-fcfs-alpaca")
fastserve_fcfs = fastserve_fcfs + [fastserve_fcfs[-1]] * 1
_, fastserve = read_result_series("logs", "exp-fastserve-alpaca")

print(faster_transformer)

rate_y = [faster_transformer, vllm, fastserve_fcfs, fastserve]

rate_x = [1,	3,	5,	8,	11,	11.5,	12,	13,	14,	15,	17,	20,	30,	40]

print('faster_transformer', rate_y[1])


rate_x_ticks = [i * 1 for i in range(0, 41, 5)] # [4, 8, 12, 16]
...
# Save the figure
file_path = os.path.join(lib.get_exp_result_root(), "fig11_alpaca.pdf")
