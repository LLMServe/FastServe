...
rate_x_vllm = [1, 2, 4, 6, 8, 10, 11, 12]
rate_x_fcfs = [1, 10, 11, 12, 15, 16, 17]
rate_x_fastserve = [1, 10, 11, 12, 15, 16, 17, 18, 21, 23, 25]

rate_x_ticks = [i * 1 for i in range(0, 25, 4)] # [4, 8, 12, 16]
rate_y_ticks = [i * 0.1 for i in range(0, 11, 2)] # [150, 300, 450, 600]

# Plot x ticks and label
for j in range(num_subfigs):
    if j == 0:
        axes.set_xlabel(x_labels[j])
        axes.set_xlim(left=0, right=24)
...
# Save the figure
file_path = os.path.join(lib.get_exp_result_root(), "fig15_alpaca.pdf")
