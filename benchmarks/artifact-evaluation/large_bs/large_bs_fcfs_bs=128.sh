...
num_prompt_list=(200 400 500 550 650)
rate_list=(1 2 2.5 2.75 3.5)
...
for (( i=0; i<num_tests; i++ )); do
    ...
    LOG_FILE="${LOG_DIR}/exp-fcfs-${DATASET_NAME}-${num_prompt}-${rate}-bs=${MAX_BATCH_SIZE}-client-log"
    ...
    python3 fastserve/api_server/fastserve_api_server.py \
        ...
        --use-skip-join \
        --use-dummy-weights > ${LOG_DIR}/server.log &
