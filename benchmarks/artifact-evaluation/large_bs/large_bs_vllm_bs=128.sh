...
for (( i=0; i<num_tests; i++ )); do
    ...
    LOG_FILE="${LOG_DIR}/exp-vllm-${DATASET_NAME}-${num_prompt}-${rate}-bs=${MAX_BATCH_SIZE}-client-log"
    ...
    CUDA_VISIBLE_DEVICES=0 python -m vllm.entrypoints.api_server \
        ...
        --max-model-len 2048 --max-num-seqs "$MAX_BATCH_SIZE" \
        --enforce-eager --disable-custom-all-reduce \
        --device cuda --num-scheduler-steps 1 \
        --disable-log-requests > ${LOG_DIR}/server.log &
