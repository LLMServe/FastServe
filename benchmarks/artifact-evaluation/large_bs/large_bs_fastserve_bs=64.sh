...
BACKEND_PORT="10000"
MODEL_PATH="/users/wby/weights/opt-13b-swtransformer"
BLOCK_SIZE="16"
GPU_UTIL="0.9"
SWAP_SPACE="60"
SCHED_POLICY="sj-mlfq"
MAX_BATCH_SIZE="64"
MAX_TOKENS_PER_BATCH="2048"

# Define frontend parameters
TOKENIZER_PATH="/users/wby/weights/opt-13b-swtransformer"
DATASET_PATH="/users/zzl/datasets/ShareGPT_V3_unfiltered_cleaned_split.json"
PROCESS_NAME="possion"
DATASET_NAME="sharegpt"
LOG_DIR="/users/zzl/FastServe/benchmarks/artifact-evaluation/large_bs/logs"
...
num_prompt_list=(200 400 500 550 600 650 700 700)
rate_list=(1 2 2.5 2.75 3 3.5 3.75 4)
...
for (( i=0; i<num_tests; i++ )); do
    ...
    LOG_FILE="${LOG_DIR}/exp-fastserve-${DATASET_NAME}-${num_prompt}-${rate}-bs=${MAX_BATCH_SIZE}-client-log"
    ...
    CUDA_VISIBLE_DEVICES=1 python3 fastserve/api_server/fastserve_api_server.py \
        --host "$BACKEND_HOST" \
        --port "$BACKEND_PORT" \
        --model "$MODEL_PATH" \
        ...
        --use-skip-join \
        --profiling-file ./profiling-model > ${LOG_DIR}/server.log &
